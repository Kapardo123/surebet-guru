...
const ExpandedCouponView = ({ coupon, onClose }: { coupon: Coupon; onClose: () => void }) => (
          ...
          {sortedWins.map((item, index) => (
            <div key={item.id}>
              ...
              <div
                ...
                className={`flex flex-col sm:flex-row gap-1.5 sm:gap-2 md:gap-3 p-2.5 sm:p-3 md:p-3.5 bg-gradient-to-r from-card/70 to-card/40 rounded-xl transition-all duration-200 ${
                  item.type === 'coupon'
              ...
              >
                {/* Top row: number + icon + type/prediction badges */}
                <div className="flex items-center justify-between w-full sm:w-auto sm:flex-shrink-0">
                  ...
                  <div className="flex sm:hidden items-center gap-1 shrink-0 flex-wrap justify-end">
                    {getTypeBadge(item)}
                    <span className="text-[9px] font-bold text-emerald-400 bg-emerald-500/10 px-1 py-0.5 rounded-full">@ {Number(item.odds).toFixed(2)}</span>
                  </div>
                </div>
                  ...
                  {item.type === 'coupon' ? (
                    <div className="flex items-center gap-1.5 sm:gap-2 md:gap-3 min-w-0">
                      <div className="w-6 h-6 sm:w-8 sm:h-8 md:w-9 md:h-9 rounded-lg bg-gradient-to-br from-blue-500/20 to-cyan-500/20 flex items-center justify-center ring-2 ring-blue-500/30 shrink-0">
                        <PartyPopper className="w-3 h-3 sm:w-4 sm:h-4 md:w-4.5 md:h-4.5 text-blue-400" />
                      </div>
                ...
                {/* Bottom row on mobile / right side on desktop: badges */}
                <div className="flex items-center gap-1 sm:gap-1.5 md:gap-2 sm:pl-0 shrink-0 flex-wrap">
                  <span className="hidden sm:flex items-center gap-1">{getTypeBadge(item)}</span>
                  <Badge variant="win" className="text-[7px] sm:text-[8px] md:text-[9px] px-1 sm:px-1.5 md:px-2 py-0 h-4 sm:h-4 md:h-5 font-bold whitespace-nowrap">
                    {item.prediction}
                  </Badge>
                  <span className="text-[9px] sm:text-[10px] md:text-[11px] font-bold text-emerald-400 bg-emerald-500/10 px-1 sm:px-1.5 md:px-2 py-0.5 rounded-full whitespace-nowrap hidden sm:inline">@ {Number(item.odds).toFixed(2)}</span>
                  <span className="text-[7px] sm:text-[8px] md:text-[9px] text-muted-foreground/70 flex items-center gap-0.5 whitespace-nowrap">
              ...
              {item.type === 'coupon' && expandedCouponId === item.id && item.couponData && (
                <ExpandedCouponView coupon={item.couponData} onClose={() => setExpandedCouponId(null)} />
              )}
            </div>
          ))}
        </div>
          ...
          <div className="flex items-center gap-1 sm:gap-1.5 md:gap-2 opacity-70">
            ...
            {coupons.length > 0 && (
              <>
                ...
                <span className="font-semibold">Coupons ({coupons.length})</span>
              </>
            )}
            {heroPick?.status === 'won' && (
              <>
...
export default memo(RecentWins);
